...
public class cavalloN extends Pezzo {
	private ArrayList<Point>ls=new ArrayList<Point>();
	
	// costruttore : è una cella vuota
	public cavalloN(){
		super.id="cavalloN";  // prende id dalla classe madre
		super.idgruppo="N";
	}
	...
	public  void calcoloMossePossibili(int x, int y) {
		...
		if(x+2<=7 && y-1>=0 ){  // ! what bug?
			check6mossa(x, y);
		}
		if(x==2 && y==5){  // caso speciale  
			check6mossa(x, y);
			System.out.print("\n caso speciale");
	...
	public void check1mossa(int x,int y){
		...
		//1 mossa
		if(! super.c.getMatrice()[x-2][y-1].getidgruppo().equals("N")){ // può andare in questo cella solo se non c'è un suo pezzo alleato
	...
	public void check3mossa(int x,int y){
		...
		//3 mossa
		if(! super.c.getMatrice()[x-1][y+2].getidgruppo().equals("N") && x-1>=0 && y+2<=7){ // può andare in questo cella solo se non c'è un suo pezzo alleato
	...
	public void check4mossa(int x,int y){
		...
		//4 mossa
				if(! super.c.getMatrice()[x+1][y+2].getidgruppo().equals("N") ){ // può andare in questo cella solo se non c'è un suo pezzo alleato
					Point p =new Point(x+1,y+2);
					ls.add(p);
					}
	}
	...
	public void setIcona(int riga,int colonna){
		...
		b.setIcon(new ImageIcon("CavalloN_sfchiaro0000.jpg"));
		}
		else { // scuro
			b.setIcon(new ImageIcon("CavalloN_sfscuro0000.jpg"));
		}
	}
